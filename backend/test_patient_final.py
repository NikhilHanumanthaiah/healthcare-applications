...
def test_list_patients():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[PASS] Found {len(patients)} patients")
...
def test_verify_deletion(patient_id):
    ...
    try:
        ...
        if response.status_code == 404:
            print(f"[PASS] Patient is no longer accessible (soft-deleted)")
            return True
        else:
            print(f"[WARN] Patient still accessible (Status: {response.status_code})")
            return False
    except Exception as e:
        print(f"[ERROR] {e}")
        return False
...
if __name__ == "__main__":
    ...
    if passed == total:
        print("\n   SUCCESS! All tests passed! Patient module is fully functional!")
    else:
